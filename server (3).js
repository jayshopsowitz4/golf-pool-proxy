...
async function fetchESPN(majorId) {
  ...
  // Try multiple ESPN URL formats — they rotate which domains work
  const urls = [
  ...
  for (const url of urls) {
    try {
      ...
      if (res.ok) break;
      lastErr = `ESPN returned ${res.status} from ${url}`;
    } catch(e) { lastErr = e.message; }
  }
  if (!res || !res.ok) throw new Error(lastErr || 'ESPN unreachable');
  let raw = await res.json();
  // cdn.espn.com wraps data in gamepackageJSON or leaderboard key
  if (raw.gamepackageJSON) raw = raw.gamepackageJSON;
  if (raw.leaderboard) raw = raw.leaderboard;
